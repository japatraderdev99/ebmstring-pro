...
import { STRINGS, stringImage, type Attr, type StringModel } from "@/data/strings";
import { WHATSAPP_LINK } from "@/lib/site";

type Filter = "todos" | "poliester" | "multifilamento" | "conforto" | "spin" | "controle";
type Sort = "destaque" | "preco-asc" | "preco-desc";

const FILTERS: { value: Filter; label: string }[] = [
  { value: "todos", label: "Todos" },
  { value: "poliester", label: "Poliéster" },
  { value: "multifilamento", label: "Multifilamento" },
...
function Card({ s, index }: { s: StringModel; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col border border-line bg-surface-1 p-5 transition-colors duration-300 hover:border-lima"
    >
      {img ? (
        <>
          <div className="relative -mx-5 -mt-5 mb-3 overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 p-4">
              <span className="tech-label text-papel/70">{s.brand}</span>
              <h3 className="font-display text-2xl leading-none text-papel drop-shadow-md">
                {s.name}
              </h3>
            </div>
          </div>
        ...
        <div className="mb-4">
          <span className="tech-label text-faint">{s.brand}</span>
          <h3 className="font-display text-2xl leading-tight text-papel">{s.name}</h3>
